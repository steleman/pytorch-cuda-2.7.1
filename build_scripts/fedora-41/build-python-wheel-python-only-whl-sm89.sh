...
here="`pwd`"
arch="`uname -m`"
output_file="${here}/pytorch-wheel-python-whl-build-sm-${cuda_archs_enabled}.log"
...
if [ ${rc} -eq 0 ] ; then
  ts="`date +%Y%m%d%H%M%S`"
  mv build build-python3-linux-wheel-${ts}
  mv dist  dist-python3-linux-wheel-${ts}
fi
